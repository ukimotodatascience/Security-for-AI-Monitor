...
| **NIST Playbook** | [NIST AIRC](https://airc.nist.gov/docs/playbook.json) | HTTPS GET (JSONフィード) | ・NIST AI RMF Playbookの全コントロールデータを一括取得 |
...
<details>
<summary>NIST AI RMF Playbook (AIリスク管理フレームワーク)</summary>

#### NISTModel
NIST AI RMF Playbook のコントロール情報。
* `control_id` (str): コントロールID (例: `GOVERN-1.1`)
* `function` (str): ファンクション分類 (`GOVERN`, `MAP`, `MEASURE`, `MANAGE`)
* `category` (str): カテゴリ名 (Core Taxonomy)
* `subcategory` (str): サブカテゴリ定義・説明
* `description` (str): 詳細説明（About と Actions のセクションを結合したテキスト）
* `source_doc` (str): ソース文書名（デフォルト: `NIST AI RMF Playbook`）
* `section_about` (Optional[str]): このコントロールに関する詳細背景
* `section_actions` (Optional[str]): 推奨される具体的なアクション
* `section_doc` (Optional[str]): 関連ドキュメント情報
* `section_ref` (Optional[str]): 関連リファレンス情報
* `ai_actors` (List[str]): 関連する AI Actor
