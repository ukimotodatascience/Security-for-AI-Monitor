...
def export_data(base_raw_dir: str = "data/raw", output_dir: str = "data/processed", max_size_mb: float = 5.0) -> str:
    ...
    for cve in cves:
        ...
        # Attach CWE details
        cve_dict["cwes"] = [cwe_map[cwe_id] for cwe_id in cve.cwe_ids if cwe_id in cwe_map]
    ...
    standalone_ghsas = [
        g.model_dump(mode="json") for g in ghsas if not g.cve_id
    ]
    ...
    logger.info(f"Base JSON skeleton size: {current_size} bytes. Target limit: {target_max_bytes} bytes ({max_size_mb} MB limit)")
    ...
    for g in standalone_ghsas:
        time_series_items.append({
            "date": g.get("published_at"),
            "category": "ghsa_advisories",
            "data": g
        })
    ...
    for item in time_series_items:
        ...
        item_size = len(item_json.encode("utf-8")) + 2 # extra bytes for list structure formatting
    ...
    if dropped_count > 0:
        logger.warning(f"File size limit reached! Omitted {dropped_count} older items from JSON export.")
    ...
    logger.info(f"Successfully exported data mart to {output_filepath} (Actual size: {actual_size / (1024*1024):.2f} MB)")
