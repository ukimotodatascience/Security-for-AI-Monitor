...
class ArXivFetcher(BaseFetcher):
    ...
    def fetch(
        self,
        query: str = 'all:"AI security" OR all:"prompt injection"',
        limit: int = 10,
        **kwargs,
    ) -> List[ArXivModel]:
        ...
        self.logger.info(
            f"Fetching papers from arXiv with query '{query}' (URL: {full_url})..."
        )
        ...
        for entry in entries:
            try:
                ...
                self.logger.warning(
                    f"Failed to parse arXiv entry {entry.get('id')}: {e}"
                )
